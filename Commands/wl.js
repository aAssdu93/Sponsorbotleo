        ...
        if(!args[0]){
            ...
            message.guild.channels.create(target.user.username, {type: "text", permissionOverwrites: [
                ...
                channel.send(new Discord.MessageEmbed() 
                    .setTitle(`**Bienvenue dans votre salon privé, ${target.user.username}**`)
                    .setDescription(`Vous disposez maintenant de votre propre salon privé pour faire des publicités avec SponsorTest !
                    Rappelez-vous que vous devez avoir un ou plusieurs tokens de bot pour utiliser SponsorTest, si vous n'avez pas encore de token, adressez-vous à un admin pour en savoir plus.`)
                    .setColor(config.embedColor)
                )
    
                channel.send(new Discord.MessageEmbed() 
                    .setTitle("Utilisation de SponsorTest")
                    .setDescription("__Voici les commandes que vous pouvez utiliser avec SponsorTest:__")
                    ...
                    .addField(config.prefix + "pubco* token_bot", "Cette commande sert à envoyer un message à tous les membres en ligne d'un serveur.")
                    .addField(config.prefix + "puballco* token_bot", "Cette commande sert à envoyer un message à tous les membres en ligne des serveurs.")
                    .setColor(config.embedColor)
                )
            })
